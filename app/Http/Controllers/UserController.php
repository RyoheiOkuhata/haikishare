...
public function index(int $id)
{
    ...
    if  (!empty($soldOutProducts))
    {
        ...
        {
            $soldOutProducts = Product::whereIn('id', $product_id)
            ->orderBy('id', 'DESC')
            ->take(5)
            ->get();
...
//商品詳細
//----------------------------------------
public function  soldProductDetail(int $id)
{
...
if ($email_resets && !$this->tokenExpired($email_resets->created_at))
{
    ...
    DB::table('email_resets')
        ->where('token', $token)
        ->delete();
        return redirect('/TopPage')->with('flash_message', 'メールアドレスの更新が完了しました');
}else{
    ...
    return redirect('/TopPage')->with('flash_message', 'メールアドレスの更新に失敗しました。もう一度やり直してください');
}
}
//----------------------------------------
// トークンが有効期限切れかどうかチェック
